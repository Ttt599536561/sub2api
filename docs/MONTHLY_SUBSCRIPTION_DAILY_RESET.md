...
- 开发日期：2026-09-06
- 记录更新日期：2026-09-07
- 当前状态：功能实现和两轮审查修复已完成本地验证，并已启动独立的本地预览环境；两轮修复仍在工作区，未提交、未发布。
...
| 原功能文档 | `bff587089` | 本文初版；也是本次记录更新前的 `HEAD`。 |
| 两轮审查修复及本次文档更新 | 未提交的工作区改动 | 包含下文 REV-01 至 REV-10，不能仅用 `0be85e950` 或 `bff587089` 代表当前修复后的源码。 |

与线上源码基线比较，历史数据库迁移文件未修改，仅新增 `235_subscription_daily_reset.sql`。该结论来自源码比较，尚未读取生产数据库的 `schema_migrations` 或执行生产升级演练。

后续提交修复时，应在此补充实际提交号，并将以下两个目前未跟踪的回归测试一并纳入提交：
...
第二轮修复后未重新运行前端全量测试或前端生产构建，不能将 1885 项历史全量结果视为最终工作区的全量结果。本次文档更新记录已有执行证据，不重新执行这些测试。
...
以下 10 项均由本次新增代码引入，已在当前工作区修复。编号用于后续提交、问题单和回归定位，尚未关联新的 Git 提交。
...
- 初始全量服务测试记录了 5 项范围外失败：4 项 Windows 插件包文件占用错误已在原始基线复现；1 项内容审核缓存定时用例仍需单独处理。后续使用专项服务测试，没有重新证明全量服务套件通过，也没有把这些范围外文件纳入修复。
- 最终 Linux 发布镜像尚未构建验证，生产数据库备份副本升级演练及生产规模负载测试尚未执行；本地 Windows 预览不替代这些验证。
