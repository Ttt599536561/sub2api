...
本次工作的统一入口：[二次开发、上游合并与审查总记录](CHANGE_SUMMARY_2026-09-17.md)。
...
| 合并后的再次审查 | `bff0df512` | 六类边界修复与新增回归测试；这是本次最终代码基线。 |

原月卡功能开发阶段相对上述运行源码基线仅新增 `235_subscription_daily_reset.sql`，未修改历史迁移。后续合并也引入了上游新增迁移，详见总记录；这些结论来自源码比较，尚未读取生产数据库的 `schema_migrations` 或执行生产升级演练。
...
- 首次整合阶段已完成 Linux amd64、CGO=0、内嵌前端资源的完整二进制交叉编译；最终 `bff0df512` 修复版没有再次执行该 Linux 构建。发布容器镜像、生产数据库备份副本升级演练及生产规模负载测试尚未验证。
