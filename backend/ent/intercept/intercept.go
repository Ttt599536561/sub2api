...
// The SubscriptionDailyResetEventFunc type is an adapter to allow the use of ordinary function as a Querier.
type SubscriptionDailyResetEventFunc func(context.Context, *ent.SubscriptionDailyResetEventQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f SubscriptionDailyResetEventFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.SubscriptionDailyResetEventQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.SubscriptionDailyResetEventQuery", q)
}

// The TraverseSubscriptionDailyResetEvent type is an adapter to allow the use of ordinary function as Traverser.
type TraverseSubscriptionDailyResetEvent func(context.Context, *ent.SubscriptionDailyResetEventQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseSubscriptionDailyResetEvent) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseSubscriptionDailyResetEvent) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.SubscriptionDailyResetEventQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.SubscriptionDailyResetEventQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.SubscriptionDailyResetEventQuery:
		return &query[*ent.SubscriptionDailyResetEventQuery, predicate.SubscriptionDailyResetEvent, subscriptiondailyresetevent.OrderOption]{typ: ent.TypeSubscriptionDailyResetEvent, tq: q}, nil
